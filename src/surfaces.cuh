...
inline void Diffuse_BRDF(	Ray& in_ray,
	...
	init_ray(out_ray, inters_point + surf_normal * 0.00005f,
...
inline void ReflectiveDiffuse_BRDF(	Ray& in_ray,
	...
	if (r2 > refl_factor) {
		...
		init_ray(out_ray, new_orig, normalize(u * cosr1 * r2s + v * sinr1*r2s + surf_normal * __fsqrt_rn(1 - r2)));
	}
	else {
		// perfect reflection
		init_ray(out_ray, new_orig, normalize(reflect(in_ray.direction, surf_normal)));
	}
...
inline void Refractive_BRDF(Ray& in_ray,
	...
	float3 ray_oriented_norm = dot(surf_normal, in_ray.direction) < 0 ? surf_normal : -surf_normal;
	...
	{
		init_ray(out_ray, inters_point + ray_oriented_norm * REFL_BIAS,
	...
	else // refraction of light ray
	{
		...
		{//transmision
			...
			init_ray(out_ray, inters_point - ray_oriented_norm * REFL_BIAS_LOW,
				trans_ray_dir);
		}
	}
